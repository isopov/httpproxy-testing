...
//	private static int processorsCount = 0;
	...
	private static final byte[] CONNECT_OK = ("HTTP/1.0 200 Connection established" + CRLF + "Proxy-agent: ProxyLight"
			+ CRLF + CRLF).getBytes();
						...
						try {
							...
							while (selector != null) {
								...
								if (selector.selectedKeys().size() == 0) {
									...
									// Faire le menage.
									for (Iterator<Entry<String, Socket>> i = outSockets.entrySet().iterator(); i
											.hasNext();) {
										Entry<String, Socket> e = i.next();
										Socket so = e.getValue();
										long lastOp = Math.max(so.lastRead, so.lastWrite);
										if (lastOp < limit) {
											debug("processeur " + processorIdx
													+ " : Fermeture pour inactivite de la socket vers " + e.getKey());
														...
														if (step == REQUEST_STEP.STATUS_LINE) {
															...
															if (s != null) {
																if (s.length() == 0) {
																	...
																	boolean isConnect = !isGet && !isPost
																			&& "CONNECT".equals(request.getMethod());
																	...
																	if (outSocket == null) {
																		...
																		outSocket.socket.register(selector,
																				SelectionKey.OP_READ, outSocket);
																		outSockets.put(oh, outSocket);
																		debug("Ajout d'une socket vers " + oh
																				+ " sur le processeur " + processorIdx
																				+ ". Socket count=" + outSockets.size());
																	}
																	...
																	if (isConnect) {
																		...
																		send.append(url).append(" ")
																				.append(request.getProtocol())
																				.append(CRLF);
																		for (Entry<String, String> h : request
																				.getHeaders().entrySet()) {
																			send.append(h.getKey()).append(": ")
																		...
																		if (isPost) {
																			contentLength = Integer
																					.parseInt(request.getHeaders().get(
																							"Content-Length"));
																		}
																		step = contentLength == 0 ? REQUEST_STEP.STATUS_LINE
																				: REQUEST_STEP.REQUEST_CONTENT;
																	}
																} else {
																	request.addHeader(s);
																}
...
//					processorsCount--;
	...
	private static void write(Socket socket, ByteBuffer b, long when) throws IOException {
		socket.socket.write(b);
		socket.lastWrite = when;
	}

	private static int read(Socket socket, ByteBuffer b, long when) throws IOException {
